...
def train_bc(env,student,exp_name):
    ...
    for i in range(50000):
        transitions.append({'obs':np.array(log['obs'][i]),'acts':np.array(log['action'][i])})
    """
...
def expert(env,exp_name):
    ...
    for i in range(exp_config['timesteps']):
        ...
        x=np.concatenate((np.array(q),np.array(v),v_des,np.array([w_des])))
        x=x[0:37]
        #log.save()
        if done==True:
            print("***")
        ...
        pln_ctr = 0
        lag = int(update_time/sim_dt)
        gg = SoloMpcGaitGen(env.pin_robot, env.urdf_path, x, plan_freq,x[0:19], None)
        ...
        for o in range(int(exp_config['steps']*(plan_freq/sim_dt))):
                ...
                if pln_ctr == 0:
                    ...
                    pr_st = time.time()
                    xs_plan, us_plan, f_plan = gg.optimize(np.array(obs[0:19]), np.array(obs[19:37]), np.round(sim_t,3), np.array(obs[37:40]), np.array(obs[40]))
                ...
                tau = robot_id_ctrl.id_joint_torques(np.array(obs[0:19]), np.array(obs[19:37]), xs[index][:env.pin_robot.model.nq].copy(), xs[index][env.pin_robot.model.nq:].copy()\
        ...
        if not done:
            ...
            for traj in trajectories:
                print(traj['obs'].tolist())
                log.add('obs',traj['obs'].tolist())
                #log.add('action',traj['acts'].tolist())
